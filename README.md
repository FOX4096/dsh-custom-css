...
从左到右：**[文件下拉] [打开文件] [导入] [导出] [重置]**；编辑器上方的**文件条**右侧另有一个**开关胶囊**。
...
| 开关胶囊（文件条上） | 编辑器上方文件条的右侧：**临时停用当前样式表** —— 样式不再注入页面，而文件内容、当前选择、校验状态全都不动。状态写进 `active.json` 的 `disabled` 列表，因此多个浏览器与重启后一致。左侧是「CSS 图标 + 文件名 + `CSS` 徽章」，与状态点一起构成这一行的身份区 |
