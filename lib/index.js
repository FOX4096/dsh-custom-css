...
/**
 * Read the recorded active stylesheet name.
 * @returns the name, or `undefined` when unset or unusable.
 */
async function activeName() {
  ...
  try {
    ...
    return typeof active === 'string' && NAME_PATTERN.test(active) ? active : undefined;
  }
  catch {
    return undefined;
  }
...
/**
 * Record the active stylesheet name.
 * @param name - an accepted file name.
 */
async function setActiveName(name) {
  const dir = stylesDir();
  await fs.mkdir(dir, { recursive: true });
  await fs.writeFile(path.join(dir, ACTIVE_FILE), `${JSON.stringify({ active: name }, null, 2)}\n`, 'utf8');
}
...
async function handle(req, res) {
  ...
  if (method === 'GET' && route === '/list') {
    const [files, active] = await Promise.all([listFiles(), activeName()]);
    json(res, 200, { ok: true, dir: stylesDir(), files, active: active ?? null });
    return;
  }
