	...
	factory: (require) => {
		...
		function CustomCssRow() {
			...
			React.useEffect(() => {
				...
				readout.textContent = '把指针移到要改的元素上';
				// The help lives on hover instead of sitting in the bar: the bar has to stay
				// short, and the keys are only needed while learning them.
				const helpTrigger = document.createElement('span');
				helpTrigger.className = 'dshCc_pickHelpTrigger';
				helpTrigger.textContent = '快捷键';
				const help = document.createElement('div');
				help.className = 'dshCc_pickHelp';
				for (const line of ['↑ 上一个层级（父元素）', '↓ 下一个层级（第一个子元素）', 'Enter 确认当前高亮', 'Esc 取消']) {
					const row = document.createElement('div');
					row.className = 'dshCc_pickHelpRow';
				...
				slider.type = 'range';
				slider.className = 'dshCc_pickSlider';
				slider.min = '0';
				slider.max = '6';
				slider.value = '0';
				slider.title = '层级深度：拖动在祖先链上移动，0 表示指针下的元素';
				let session = null;
				slider.addEventListener('input', () => session?.setDepth(Number(slider.value)));
				bar.appendChild(readout);
				bar.appendChild(helpTrigger);
