...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function walkStructure(text, visit) {
			...
			while (index < text.length) {
				...
				if (ch === '"' || ch === "'") {
					...
					// skipString stops short of the end only when it found the closing
					// quote; ending on any other character means the string is open.
					if (stop === text.length && text[text.length - 1] !== ch) open = true;
					index = stop;
					continue;
				}
				if (ch === '/' && text[index + 1] === '*') {
					...
					if (depth === 0 && selector !== '' && !selector.startsWith('@')) {
						...
						while (scan < css.length && inner > 0) {
							...
							if (innerCh === '/' && css[scan + 1] === '*') {
								const end = css.indexOf('*/', scan + 2);
								scan = end < 0 ? css.length : end + 2;
								continue;
							}
							if (innerCh === '{') inner += 1;
		 ...
		 * `/*`. Every scanner in this file has to skip them, or `content: "}"` looks
		 * like a closing brace and `url("data:…;base64,…")` looks like two declarations.
