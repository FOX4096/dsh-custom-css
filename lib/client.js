	...
	factory: (require) => {
		...
		async function bootstrap() {
			try {
				...
				sheetOnDisk.set(active, sheet.css);
		...
		/**
		 * What this plugin believes sits on disk, per sheet.
		 *
		 ...
		 * failure here that loses data instead of looking untidy, so a write is worth one read
		 * to confirm the ground has not moved.
		 */
		const sheetOnDisk = new Map();

		/**
		 * The text on disk, when it is no longer what this plugin last saw.
		 * @param name - the sheet to ask about.
		 * @returns the disk text when it differs, or null when it agrees or was never seen.
		 */
		async function changedOnDisk(name) {
			const expected = sheetOnDisk.get(name);
			if (expected === undefined) return null;
			const sheet = await api('/read?name=' + encodeURIComponent(name));
			return sheet.css === expected ? null : sheet.css;
		}
		...
		async function writeSheet(name, css) {
			try {
				const disk = await changedOnDisk(name);
				if (disk !== null) {
					store.set({ conflict: { name, disk }, error: '' });
					return false;
				}
				await post('/write', { name, css });
				sheetOnDisk.set(name, css);
				store.set({ error: '' });
				return true;
			}
			catch (error) {
		...
		function CustomCssRow() {
			...
			const flushSave = async () => {
				...
				return writeSheet(pending.name, pending.css);
			};
			...
			/**
			 * Look for an outside edit to the active sheet, and raise it if there is one.
			 *
			 * Writes are checked anyway (see `writeSheet`), but that check only fires when the
			 * user types — and by then the other copy has been sitting there. This runs when the
			 * window comes back into view, which is exactly the moment an edit made in an
			 * external editor becomes a collision waiting to happen.
			 */
			const checkForOutsideEdit = async () => {
				const current = store.snapshot();
				if (current.mode !== 'files' || current.active === null || current.conflict !== null) return;
				try {
					const disk = await changedOnDisk(current.active);
					if (disk !== null) store.set({ conflict: { name: current.active, disk } });
				}
				catch {
			...
			const takeDiskCopy = () => {
				...
				sheetOnDisk.set(conflict.name, conflict.disk);
				store.set({ conflict: null, error: '', css: conflict.disk });
				applySheet(conflict.name, conflict.disk, store.snapshot().disabled);
				setSaved(true);
				setSuggest(null);
			};
			 ...
			 *
			 * The disk text is adopted as the baseline first — that is what the write is about to
			 * replace — so the confirmation cannot trip over the very change it is confirming.
			 */
			...
			const restoreVersion = async (stamp) => {
				...
				try {
					...
					if (store.snapshot().active === name) {
						sheetOnDisk.set(name, sheet.css);
