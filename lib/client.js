...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Absorbs the editor's pending debounced write, if one is armed.
		 *
		 * The row owns the debounce and the picker does not: without this, an edit still
		 * inside its 400 ms window fires after a picked rule is written and puts the old
		 * text back — the pick is lost, with no error anywhere.
		 */
		let absorbPendingWrite = null;
		...
		function editSheetText(css) {
			if (absorbPendingWrite !== null) absorbPendingWrite();
			...
			post('/write', { name: current.active, css }).catch(error => {
		...
		function CustomCssRow() {
			...
			// debounced write out of the way first (see absorbPendingWrite).
			React.useEffect(() => {
				absorbPendingWrite = () => {
					clearTimeout(saveTimer.current);
					pendingSave.current = null;
