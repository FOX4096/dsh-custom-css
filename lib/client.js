...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function editSheetText(css) {
			...
			post('/write', { name: current.active, css }).then(
				() => {
					if (handoff !== null) handoff.settled(true);
				},
				(error) => {
					store.set({ error: '保存失败：' + (error?.message ?? error) });
					// A failed outside write settles too: silence would leave the footer
					// waiting on a write that is already over.
					if (handoff !== null) handoff.settled(false);
				},
			);
		}
