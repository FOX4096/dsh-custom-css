...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function startPickSession(host) {
			...
			// The slider walks up from the element a click chose, so its zero stays put no
			// matter how far the keys have already moved the selection.
			let anchor = null;
			const lock = (node, keepAnchor = false) => {
				if (node === undefined || node === null) return;
				selected = node;
				hovered = node;
				if (!keepAnchor) {
				...
				host.showBox(node);
				if (host.onSelect !== undefined) host.onSelect(node, depth, false);
			};
			const setDepth = (levels) => {
				if (stopped || anchor === null) return;
				let node = anchor;
				let walked = 0;
				for (let step = 0; step < levels && node !== null; step += 1) {
			...
			const step = (direction) => {
				...
				const next = host.relative(selected, direction);
				if (next === null) return;
				// The slider counts levels above the anchor, so walking up has to move both
				// the selection and the knob; sideways leaves the level alone.
				if (direction === 'parent') depth += 1;
				else if (direction === 'child') depth = Math.max(0, depth - 1);
				lock(next, true);
			};
			const confirm = () => {
				if (stopped || selected === null) return;
