...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Declarations whose `var()` cannot resolve where their rule applies.
		 *
		 ...
		 * declarations and says nothing, which is the worst possible way to find out. So this
		 * asks the document: match the rule, read the property off one of the elements it hits.
		 *
		 * Silence is deliberate in two cases — a rule that matches nothing (there is nobody to
		 * ask; a sheet styles states and pages that are not on screen) and a `var()` carrying a
		 * fallback (there is nothing to lose). A check that cries wolf about those would be
		 * turned off within a day, and then it would catch nothing at all.
		 * @param css - the sheet text.
		 * @param doc - the document to ask.
		 * @returns `{ line, message }` records, in sheet order.
		 */
		function checkVariables(css, doc) {
			if (doc === undefined || doc === null || typeof doc.querySelectorAll !== 'function') return [];
			const found = [];
			for (const rule of parseRules(css).slice(0, VAR_CHECK_RULES)) {
				let elements;
				try {
					elements = [...doc.querySelectorAll(rule.selector)].slice(0, VAR_CHECK_SAMPLES);
				}
				catch {
		...
		function CustomCssRow() {
			...
			/** `var()` problems, which need the document and therefore arrive asynchronously. */
			const [variableIssues, setVariableIssues] = React.useState([]);
			/**
			...
			// The `var()` check reads the document, so it waits for a pause rather than running
			// per keystroke, and it only runs for a sheet that parses: syntax first, then the
			// quieter question (see `checkVariables`).
			React.useEffect(() => {
				...
				const timer = setTimeout(() => {
					setVariableIssues(validateCss(state.css).length > 0 ? [] : checkVariables(state.css, document));
				}, VAR_CHECK_DELAY_MS);
				return () => {
					clearTimeout(timer);
				};
			...
			const issues = [...validateCss(state.css), ...variableIssues]
