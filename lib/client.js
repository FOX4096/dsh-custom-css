	...
	factory: (require) => {
		...
		function startPicking() {
			...
			const session = startPickSession({
				...
				// page's own click is left alone (see onSelect in the session).
				onSelect: (node) => {
					place(node, locked);
					parts.slider.value = '0';
					inspect(node);
				},
				hideBox: () => {
					box.style.width = '0';
		...
		function startPickSession(host) {
			...
			const lock = (node, keepAnchor = false) => {
				...
				selected = node;
				hovered = node;
				if (!keepAnchor) anchor = node;
				host.showBox(node);
				if (host.onSelect !== undefined) host.onSelect(node);
			};
			const setDepth = (depth) => {
				if (stopped || anchor === null) return;
				let node = anchor;
				for (let step = 0; step < depth && node !== null; step += 1) {
					node = host.relative(node, 'parent');
				}
				lock(node, true);
			};
			const step = (direction) => {
				if (stopped || selected === null) return;
				lock(host.relative(selected, direction), true);
			};
			const confirm = () => {
				if (stopped || selected === null) return;
				const node = selected;
