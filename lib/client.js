...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ROW_CSS = [
			...
			'padding:0 14px;border:none;border-radius:18px;background:transparent;color:var(--dsw-alias-label-primary);',
		...
		function startPicking() {
			...
			const session = startPickSession({
				...
				// The page keeps its clicks: this picker selects by hovering and commits on
				// Enter or on its own button.
				clickToPick: false,
		...
		function startPickSession(host) {
			const picksOnClick = host.clickToPick !== false;
			let stopped = false;
			...
			const stop = () => {
				...
				if (picksOnClick) host.unlisten('click', onClick, true);
