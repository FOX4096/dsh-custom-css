...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function readDeclarations(css, rule) {
			...
			// even though the writer would have normalised it on the next edit.
		...
		function writeDeclaration(css, rule, name, value) {
			...
			// Comments are blanked (not removed) before splitting: a chunk like
			// `/* note */ display: block` starts with `/`, so the property used to go
			// unrecognised and a second copy got appended instead of replacing it.
			const scan = body.replace(/\/\*[\s\S]*?\*\//g, comment => ' '.repeat(comment.length));
			const chunks = [];
			let cursor = 0;
			while (cursor < scan.length) {
				const semi = scan.indexOf(';', cursor);
				const stop = semi < 0 ? scan.length : semi;
				const chunk = body.slice(cursor, stop);
				const match = /^\s*([-\w]+)\s*:/.exec(scan.slice(cursor, stop));
				chunks.push({
					start: cursor,
					end: stop,
					chunk,
				...
				cursor = semi < 0 ? scan.length : semi + 1;
			}

			const matching = chunks.filter(entry => entry.target);
			if (matching.length === 1 && value !== '') {
				// The common case: rewrite the value where it already sits, keeping any
				// comment that shares its chunk.
				const only = matching[0];
				const head = /^((?:\s*\/\*[\s\S]*?\*\/)?\s*[-\w]+\s*:\s*)/.exec(only.chunk)[1];
				const nextBody = body.slice(0, only.start) + head + value + body.slice(only.end);
			...
			for (const entry of chunks) {
				...
				const comments = entry.chunk.match(/\/\*[\s\S]*?\*\//g);
				if (comments !== null) keptParts.push(comments.join('\n') + '\n');
			}
		...
		function CustomCssRow() {
			...
			const persist = (css) => {
				...
				saveTimer.current = setTimeout(() => {
					void flushSave().then(() => {
						// A write that lands after a newer edit must not report "saved": the
						// indicator would then be lying until that newer write finishes.
						if (epoch === saveEpoch.current) setSaved(true);
					});
				}, WRITE_DELAY_MS);
			};
					...
					openRule === null ? null : React.createElement(
						...
						React.createElement(
							...
							React.createElement('input', {
								...
								'aria-label': '选择器',
								spellCheck: false,
								value: selectorDraft === null ? (openSelector ?? '') : selectorDraft,
								onChange: (event) => {
