...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildPickPanel() {
			...
			const readout = chrome('span', 'dshCc_pickPanelInfo', '把指针移到要改的元素上');
			...
			const pause = chrome('button', 'dshCc_pickBarBtn', '暂停拾取');
			pause.type = 'button';
			pause.title = '把鼠标还给你：暂停期间可以正常点界面，拾取面板与高亮保留';
			...
			return { panel, readout, list, tokens, slider, cancel, pause, up, down, insert };
		}
		...
		function startPicking() {
			...
			const session = startPickSession({
				...
				// Clicking the highlighted element confirms it; the panel's pause button
				// hands the page its clicks back for navigating somewhere else.
				clickToPick: true,
			...
			parts.pause.addEventListener('click', () => {
				paused = !paused;
				session.setClickToPick(!paused);
				parts.pause.textContent = paused ? '继续拾取' : '暂停拾取';
				parts.panel.setAttribute('data-paused', paused ? 'true' : 'false');
			});
		...
		function startPickSession(host) {
			let picksOnClick = host.clickToPick !== false;
			let stopped = false;
			// `base` is what the pointer chose; the keyboard and the slider move relative
			// to it, so one can always come back to where the mouse was.
			let base = null;
			let selected = null;
			const stop = () => {
				if (stopped) return;
			...
			/**
			 * Take the page's clicks, or give them back.
			 *
			 * Clicking the highlighted element is how most people expect to confirm,
			 * but a picker that never lets go makes the rest of the interface
			 * unreachable — so the session can be paused, which keeps the highlight and
			 * the panel while the page goes back to normal.
			 * @param next - true to intercept clicks again.
			 */
			const setClickToPick = (next) => {
				if (stopped || next === picksOnClick) return;
				picksOnClick = next;
				if (next) host.listen('click', onClick, true);
				else host.unlisten('click', onClick, true);
			};
			const show = (node) => {
			...
			const setDepth = (depth) => {
				if (stopped) return;
				const node = fromBase(depth);
				if (node !== null) show(node);
			};
			...
			const onMove = (event) => {
				...
				const node = host.elementAt(event.clientX, event.clientY);
				if (node === undefined || node === null) return;
				base = node;
				host.onBase?.(node);
				show(node);
			...
			if (picksOnClick) host.listen('click', onClick, true);
			host.listen('keydown', onKey, true);
			return { stop, step, confirm, setDepth, setClickToPick };
		}
