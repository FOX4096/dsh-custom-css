...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function insertPickedRule(selector, declarations) {
			...
			if (existing >= 0) {
				pendingOpenCaret = rules[existing].bodyStart;
			}
			else {
			...
			// The gesture ends where the work continues: settings reopened on the new rule,
			// caret inside it, ready to type.
			openSettingsSurface();
		}
		...
		function CustomCssRow() {
			...
			// The picker runs at module scope, so a rule it appended is remembered there
			// and opened here as soon as this row is on screen again.
			React.useEffect(() => {
