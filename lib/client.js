...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ROW_CSS = [
			...
			// Scrollbars are NOT styled here on purpose: DSH styles every scrollbar from one
			// global stylesheet (`::-webkit-scrollbar` + `--dsh-scrollbar-*` tokens), and this
			// plugin's job is not to interfere with it.
			//
			// The `scrollbar-width: thin` that used to sit on this rule (and on `.dshCc_panel`)
			// was exactly that interference: in Chromium, setting a standard scrollbar property
			// makes the engine IGNORE `::-webkit-scrollbar` for that element — so these menus and
			// the rule panel fell back to the platform bar (square, grey) while the rest of the
			// app stayed styled. The suite's scrollbar audit fails if either comes back.
