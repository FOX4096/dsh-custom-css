...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ROW_CSS = [
			...
			'.dshCc_pickBar{position:fixed;z-index:2147483001;left:50%;bottom:24px;transform:translateX(-50%);pointer-events:none;padding:6px 12px;border-radius:18px;background:var(--dsw-specific-menu);box-shadow:var(--dsw-elevation-prominent);color:var(--dsw-alias-label-primary);font-size:13px}',
		...
		function startPickSession(host) {
			...
			const onClick = (event) => {
				...
				const node = host.elementAt(event.clientX, event.clientY);
				stop();
				if (node !== undefined && node !== null) host.onPick(node);
			};
			const onKey = (event) => {
		...
		function CustomCssRow() {
			...
			React.useEffect(() => {
				...
				bar.textContent = '点击你要修改的元素 · Esc 取消';
				...
				const session = startPickSession({
					...
					showBox: (node) => {
						...
						box.dataset.label = tag + ' · ' + Math.round(rect.width) + '×' + Math.round(rect.height);
					},
					hideBox: () => {
						box.style.width = '0';
						box.style.height = '0';
