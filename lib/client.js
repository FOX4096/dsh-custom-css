...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ROW_CSS = [
			...
			'.dshCc_propGrid{box-sizing:border-box;display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:6px 10px;width:100%;min-width:0}',
			...
			'.dshCc_tplGrid{box-sizing:border-box;display:grid;grid-template-columns:repeat(auto-fill,minmax(104px,1fr));gap:6px;width:100%;min-width:0}',
			...
			'.dshCc_propValue{flex:1;min-width:0;max-width:100%;box-sizing:border-box;height:24px;padding:0 6px;',
			...
			'.dshCc_propText{flex:1;min-width:0;max-width:100%;box-sizing:border-box;height:24px;padding:0 6px;',
			'border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-module-platform);',
			...
			'.dshCc_addProp{box-sizing:border-box;display:block;margin:2px 0 0;width:100%;max-width:240px;height:24px;padding:0 6px;',
			'border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-module-platform);',
			'color:var(--dsw-alias-label-primary);font:inherit;font-size:11px;cursor:pointer}',
			...
			'.dshCc_tpl{display:flex;align-items:center;gap:6px;width:100%;min-width:0;min-height:26px;padding:4px 8px;',
		...
		const PROPERTY_UI = [
			...
			{ group: '布局', name: 'inset', label: '四边偏移', seed: '0', hint: '0 / 8px 12px' },
			...
			{ group: '布局', name: 'aspect-ratio', label: '宽高比', seed: '16 / 9', hint: '16 / 9 / 1' },
			...
			{ group: '弹性与对齐', name: 'gap', label: '间距', seed: '8px', hint: '8px / 8px 12px' },
			{ group: '弹性与对齐', name: 'row-gap', label: '行间距', seed: '8px', hint: '8px' },
			{ group: '弹性与对齐', name: 'column-gap', label: '列间距', seed: '8px', hint: '8px' },
			{ group: '弹性与对齐', name: 'flex', label: '弹性简写', seed: '1 1 auto', hint: '1 1 auto / none' },
			...
			{ group: '间距', name: 'margin', label: '外边距', seed: '0', hint: '0 / 8px / 8px 12px' },
			...
			{ group: '间距', name: 'padding', label: '内边距', seed: '8px 12px', hint: '8px / 8px 12px' },
			...
			{ group: '描边与圆角', name: 'border-radius', label: '圆角', seed: '8px', hint: '8px / 999px' },
				...
				React.createElement(
					...
					openRule === null ? null : React.createElement(
						...
						React.createElement(
							...
							// Grouped menu: 112 entries in one flat list is a wall of text, and an
							// empty group is dropped so a fully-used rule shows no blank headers.
							...PROPERTY_GROUPS
								.map(group => ({
									group,
									items: PROPERTY_UI.filter(entry => entry.group === group
										&& !(declarations?.has(entry.name) ?? false)),
								}))
								.filter(bucket => bucket.items.length > 0)
								.map(bucket => React.createElement(
									'optgroup',
									{ key: bucket.group, label: bucket.group },
									...bucket.items.map(entry => React.createElement(
										'option',
