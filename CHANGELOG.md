...
- **分量名改为浅底小标签**：`放大` / `收缩` / `基准尺寸`、`上` / `右` / `下` / `左` 这些分量名此前是纯文本，和字段值混在一起不易分辨；现在用 DSH 自带的标签底色 `--dsw-alias-markdown-tag` + `label-secondary` 文字 + 6px 圆角做成 chip（实测底色 `#f1f3f5`、文字 `#40454d`、高 18px）。
- **修掉控件被压缩的高度 bug**：规则面板是 `max-height:260px` 的纵向 flex 容器，子元素默认 `flex-shrink:1` —— 内容一超高，面板的直接子元素就被挤扁（选择器字段实测被压到 **26px**，而样式表写的是 36px）。现在 `.dshCc_panel>*{flex:none}`，行高由样式表决定、超出部分由面板滚动。实测统一为：下拉触发 / 按钮 / 行内文件名输入 / 属性值输入 / 分量输入 / 选择器字段 **全部 36px**，代码编辑区 **140px**。
- **代码编辑区最小高度恢复 140px**：随面板下移时我误把 `min-height` 从 140 改成 120，导致拖拽手柄能把编辑区缩到比它自身高度还小。
- **行内文件名输入框（新建…）对齐到 36px 胶囊**：此前是 28px / 8px 圆角 / `bg-layer-1`，与同行的 36px 下拉触发按钮明显不成套；现在 36px / 18px 圆角 / `bg-module-platform` / 14px 字号；同行的动作按钮统一 36px 高、14px 字号（仍保持 8px 直角矩形，与"字段=胶囊、动作=矩形"的层级一致）。
- **选择器字段与关闭按钮合为一个控件**：`×` 从输入框右侧的独立按钮改为**框内的圆形按钮**，输入框因此铺满面板宽度（实测 564px，与属性网格同宽）；聚焦环移到外层字段，输入框本身无边框。
- **“＋ 添加属性…”移到属性列表下方**：先读规则已声明的属性、再往下添加，符合操作顺序；仍在模板区之前。
- **“＋ 添加属性…”改为两端对齐**：铺满面板内容宽度（此前被限制在 300px），文案靠左、箭头贴右端 —— 实测控件宽 564px，与属性网格同宽。
